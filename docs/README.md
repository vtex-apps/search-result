...
| `excludedPaymentSystems`               | `string[]`                 | List of payment systems that should not be taken into account to determine the installments option to be returned. This is only taken into account if `installmentCriteria` is set to `MAX_WITHOUT_INTEREST` or `MAX_WITH_INTEREST`.                                  | `undefined` |
| `includedPaymentSystems`               | `string[]`                 | List of payment systems that should be taken into account to determine the installments option to be returned. This is only taken into account if `installmentCriteria` is set to `MAX_WITHOUT_INTEREST` or `MAX_WITH_INTEREST`. If this is not set, **all** payment systems are included.                                  | `undefined` |
